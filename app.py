...
if vis_type != "Feature Visualization by Optimization":
    ...
    if uploaded_file is not None:
        ...
        if vis_type == "Feature Maps":
            ...
            def visualize_feature_maps(feature_maps, num_maps=16):
                ...
                # Display the grid
                st.image(grid_np, caption=f"Feature Maps from {layer_name}", use_column_width=True)
            ...
            st.image(saliency_map_np, caption="Saliency Map", use_column_width=True, clamp=True)
            ...
            st.image(grad_cam, caption="Grad-CAM Heatmap", use_column_width=True, clamp=True)
            st.image(superimposed_img, caption="Superimposed Grad-CAM", use_column_width=True, clamp=True)
    ...
    st.image(optimized_image, caption="Optimized Image for Feature Visualization", use_column_width=True)
